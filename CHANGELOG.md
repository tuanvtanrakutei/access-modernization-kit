...
- Merged the `access-modernize` pipeline into this package as a second, independently invokable pipeline (`plugins/ak/modernize/`). Six stages carry a project from six-phase output through a working Django REST + React implementation, one screen at a time, with coverage gates tracing every legacy artifact to implemented code. Running one pipeline never auto-triggers the other. `bootstrap-project`, `modernize-screen`, `validate-docs`, and `triage-suite` live at `plugins/ak/skills/` — the same folder as the six-phase skill, not nested under `modernize/` — so every skill works on **both Claude Code and Codex CLI**, one install, no extra steps: Codex's manifest already points at that exact path (`"skills": "./skills/"`, enforced by `validate_structure.py`), confirmed against a real 2.7.3 install cached on the author's own machine (`~/.codex/plugins/cache/access-modernization-kit/ak/2.7.3/`), which showed the exact fault this fixes. `plugins/ak/.codex-plugin/plugin.json` also now declares `"commands": "./commands/"` and `"hooks": "./hooks/hooks.json"`, mirroring the Claude Code manifest — unlike `skills`, this is **unconfirmed**, since `ak` never declared either field before this merge and no prior cache exists to check against; the fields cost nothing if Codex's runtime does not read them.
