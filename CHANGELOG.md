...
- Merged the `access-modernize` pipeline into this package as a second, independently invokable pipeline (`plugins/ak/modernize/`). Six stages carry a project from six-phase output through a working Django REST + React implementation, one screen at a time, with coverage gates tracing every legacy artifact to implemented code. Running one pipeline never auto-triggers the other. **Claude Code only**: `plugins/ak/.claude-plugin/plugin.json` declares the pipeline's `skills`/`commands`/`hooks` paths additively; `plugins/ak/.codex-plugin/plugin.json`'s `skills` field is a single static path enforced by `validate_structure.py`, with no confirmed way to add a second path, so Codex CLI installs get six-phase investigation only for now.
