...
- The shipped example fixture `examples/minimal-app/` moves to the new layout, so a new user copying it gets the shape `init` actually creates rather than a workspace the tool no longer produces. Legacy coverage does not go with it: one test converts a copy back to `sources/` and runs the real CLI over it end to end, because the whole point of reading both layouts is that a workspace already under way keeps working.

### Fixed

- **A form and a report may share a name, and keying on the name alone silently dropped five objects — four of them print launchers, which corrupted a published finding.** Access permits a form and a report to carry the same name, and the A05 frontend does it four times (`酒アイテム別確認表`, `青果アイテム別確認表`, `雑貨Ⅱアイテム別確認表`, `冷凍品引渡表`). `derive_graph_facts.py` keyed its object map, its text corpus and its distilled fact filenames on `(database, name)`, so the second object of each pair replaced the first: 118 forms and reports became 113 distilled facts, and the four launcher forms' text was never scanned. Their `DoCmd.OpenReport` calls therefore went uncounted, and the Phase 2 figure "57 objects are referenced by nothing" — a headline finding, quoted into the QA report — was inflated by twenty. The real number is 37. Everything is now keyed by `(database, kind, name)`, the kind appears in the fact filename, and a reference to a shared name records an edge to **both** objects rather than guessing one, because the text genuinely does not say which is meant. Derived output went from 325 nodes / 756 edges to 325 / 974: the extra edges were always there and had never been read.
