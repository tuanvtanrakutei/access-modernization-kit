...
| `$ak acquire <APP_ID>` | Plan or run declared acquisition adapters to create an unapproved canonical bundle; managed Access requires explicit `access_snapshot_extract` authorization and operates only through a verified disposable snapshot. Run `$ak preflight <APP_ID>` first; managed Access acquisition needs Microsoft Access/ACE already installed and COM/DAO-registered on the host, which the kit detects but never installs. |
